...
This folder contains Labour public-event extracts and the scripts used to
recreate and analyse them.
...
- Event IDs do not track start dates: events for a single campaign window are
  scattered across a wide ID band, so the whole band must be scanned and then
...
- `profile_canvassing_events.py` - ranks wards by Labour canvassing volume in the
  ...
  2026 Senedd/Holyrood elections were national). Reports each ward's events vs
  the average in-scope ward. Outputs `ward_canvassing_frequency_2026.csv` and
  `profile_canvassing_events_summary.json`.
- `chart_for_welsh_labour/plot_wales_canvassing_heatmap.py` - joins the Welsh
  rows from `ward_canvassing_frequency_2026.csv` to the ONS December 2025 ward
  boundaries and renders a Wales-only choropleth by canvassing event density
  (events per sq km). Outputs
  `chart_for_welsh_labour/wales_canvassing_heatmap_2026.csv` and
  `chart_for_welsh_labour/wales_canvassing_heatmap_2026.png`.
- `plot_canvassing_vs_voteshare.py` - scatters canvassing intensity per English
  ...
  so canvassing cannot have inflated it); Green and Reform are from the 2026
  contest (Reform barely stood pre-2026). Outputs:
  `canvassing_vs_voteshare.png` (3 panels: a=Labour/last, b=Green/2026,
  c=Reform/2026) - binned scatters (wards sorted by x, grouped into ~100-ward
  bins, one dot per bin = bin mean; trend lines and r use the full ward-level
  data). `canvassing_green_vs_reform_2026.png` - ward-level scatter on Green (x)
  x Reform (y) 2026 shares, point size = canvassing events, colour = Labour
  share at the last election. Also `canvassing_vs_voteshare.csv`
  ...
  - Partials - Green/Reform-2026 effects net of Labour's own prior strength.
    ...
    strength (green r~+0.23 partial), and less where Reform was strong
    (~-0.20).
