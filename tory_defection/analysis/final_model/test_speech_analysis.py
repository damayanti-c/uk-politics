"""
Test Speech Analysis Using TF-IDF
==================================

Analyzes 10 years of Hansard speeches for 2024+ sitting Conservative MPs.
...
test_data_csv = Path(__file__).parent / "test_data.csv"
output_csv = Path(__file__).parent / "test_speech_features.csv"
...
def main():
    print("=" * 80)
    print("TEST SPEECH ANALYSIS - TF-IDF (2024+ Sitting MPs)")
    print("=" * 80)
    print()

    # Load test data to get MP list
    print("Loading test data...")
    test_data = pd.read_csv(test_data_csv)
    mp_names = set(test_data['name'].values)
    print(f"  {len(mp_names)} MPs in test data")
    ...
    # Now filter to test MPs for analysis
    print("\nFiltering to test MPs...")
    ...
    print(f"  {len(speeches_df)} speeches from test MPs")
