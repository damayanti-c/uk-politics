...
output_path = Path(__file__).parent / "sitting_ministerial_tenure.csv"
...
# Impute for remaining missing MPs (use 0 since we can't distinguish from no experience)
...
if still_missing:
    print(f"\nImputing 0 ministerial years for {len(still_missing)} missing MPs (API not found):")
...
print(f"MPs with imputed data: {len(still_missing) if still_missing else 0}")
